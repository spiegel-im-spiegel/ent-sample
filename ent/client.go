...
type Client struct {
	...
	// BinaryFile is the client for interacting with the BinaryFile builders.
	BinaryFile *BinaryFileClient
...
func (c *Client) init() {
	...
	c.BinaryFile = NewBinaryFileClient(c.config)
...
func (c *Client) Use(hooks ...Hook) {
	c.BinaryFile.Use(hooks...)
	c.User.Use(hooks...)
}

// BinaryFileClient is a client for the BinaryFile schema.
type BinaryFileClient struct {
	config
}

// NewBinaryFileClient returns a client for the BinaryFile from the given config.
func NewBinaryFileClient(c config) *BinaryFileClient {
	return &BinaryFileClient{config: c}
}

// Use adds a list of mutation hooks to the hooks stack.
// A call to `Use(f, g, h)` equals to `binaryfile.Hooks(f(g(h())))`.
func (c *BinaryFileClient) Use(hooks ...Hook) {
	c.hooks.BinaryFile = append(c.hooks.BinaryFile, hooks...)
}

// Create returns a create builder for BinaryFile.
...
// CreateBulk returns a builder for creating a bulk of BinaryFile entities.
...
// Update returns an update builder for BinaryFile.
func (c *BinaryFileClient) Update() *BinaryFileUpdate {
	mutation := newBinaryFileMutation(c.config, OpUpdate)
	return &BinaryFileUpdate{config: c.config, hooks: c.Hooks(), mutation: mutation}
}
...
// Delete returns a delete builder for BinaryFile.
...
// Query returns a query builder for BinaryFile.
...
// Get returns a BinaryFile entity by its id.
...
func (c *BinaryFileClient) GetX(ctx context.Context, id int) *BinaryFile {
...
// QueryOwner queries the owner edge of a BinaryFile.
func (c *BinaryFileClient) QueryOwner(bf *BinaryFile) *UserQuery {
	query := &UserQuery{config: c.config}
	query.path = func(ctx context.Context) (fromV *sql.Selector, _ error) {
		id := bf.ID
		step := sqlgraph.NewStep(
			sqlgraph.From(binaryfile.Table, binaryfile.FieldID, id),
...
func (c *BinaryFileClient) Hooks() []Hook {
	return c.hooks.BinaryFile
}
...
func (c *UserClient) QueryOwned(u *User) *BinaryFileQuery {
	query := &BinaryFileQuery{config: c.config}
	query.path = func(ctx context.Context) (fromV *sql.Selector, _ error) {
		id := u.ID
		step := sqlgraph.NewStep(
			sqlgraph.From(user.Table, user.FieldID, id),
			sqlgraph.To(binaryfile.Table, binaryfile.FieldID),
