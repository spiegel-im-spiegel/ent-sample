...
func (uq *UserQuery) QueryOwned() *BinaryFileQuery {
	query := &BinaryFileQuery{config: uq.config}
...
func (uq *UserQuery) WithOwned(opts ...func(*BinaryFileQuery)) *UserQuery {
	query := &BinaryFileQuery{config: uq.config}
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if query := uq.withOwned; query != nil {
		...
		for i := range nodes {
			...
			nodes[i].Edges.Owned = []*BinaryFile{}
		}
		query.withFKs = true
		query.Where(predicate.BinaryFile(func(s *sql.Selector) {
		...
		for _, n := range neighbors {
			fk := n.user_owned
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_owned" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_owned" returned %v for node %v`, *fk, n.ID)
			}
